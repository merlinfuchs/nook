...
func (w Thing) ID() common.ID {
	switch w.Type {
	case TypeString:
		id, _ := strconv.ParseInt(w.Value.(string), 10, 64)
		return common.ID(id)
	case TypeInt:
		return common.ID(w.Value.(int64))
	case TypeFloat:
		return common.ID(int64(w.Value.(float64)))
	case TypeBool:
		if w.Value.(bool) {
			return 1
		}
...
func (w Thing) Perform(other Thing, op Operation) Thing {
	switch op {
	case OperationOverwrite:
		return other
	case OperationAppend:
		return w.Append(other)
	case OperationPrepend:
		return other.Append(w)
